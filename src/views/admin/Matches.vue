<template>
  <div class="p-6">
      ...
      >
        ...
        <Column field="equipoLocalId" header="Equipo Local" sortable>
          <template #body="slotProps">
            <div class="flex items-center">
              <img 
                v-if="getTeam(slotProps.data.equipoLocalId)?.escudoUrl"
                :src="getTeam(slotProps.data.equipoLocalId)?.escudoUrl"
                :alt="getTeam(slotProps.data.equipoLocalId)?.nombre"
        ...
        <Column field="equipoVisitanteId" header="Equipo Visitante" sortable>
          <template #body="slotProps">
            <div class="flex items-center">
              <img 
                v-if="getTeam(slotProps.data.equipoVisitanteId)?.escudoUrl"
                :src="getTeam(slotProps.data.equipoVisitanteId)?.escudoUrl"
                :alt="getTeam(slotProps.data.equipoVisitanteId)?.nombre"
                class="w-8 h-8 object-contain mr-2"
              />
        ...
        <Column header="Acciones" :exportable="false" style="min-width:8rem">
          <template #body="slotProps">
            <div class="flex gap-2">
              <Button 
                icon="pi pi-pencil" 
                @click="editMatch(slotProps.data)"
              ...
              <Button 
                icon="pi pi-list" 
                @click="goToMatchEvents(slotProps.data.id)"
    ...
    <Dialog 
      ...
      :header="editMode ? 'Editar Partido' : 'Programar Partido'"
      modal
      class="p-fluid"
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="p-field">
          ...
          <Calendar 
            ...
            v-model="matchForm.fechaProgramacion"
            dateFormat="yy-mm-dd"
            :showIcon="true"
            class="w-full"
            required
        ...
        <div class="p-field">
          ...
          <Calendar 
            id="horaProgramacion"
            v-model="matchForm.horaProgramacion"
            timeOnly
            :showIcon="true"
            class="w-full"
            required
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        <div class="p-field">
          ...
          <Dropdown
            id="equipoLocalId"
            v-model="matchForm.equipoLocalId"
            :options="groupTeams"
            optionLabel="nombre"
            optionValue="id"
        ...
        <div class="p-field">
          ...
          <Dropdown
            ...
            v-model="matchForm.equipoVisitanteId"
            :options="groupTeams"
            optionLabel="nombre"
            optionValue="id"
      ...
      <div v-if="!editMode" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        <div class="p-field">
          ...
          <Dropdown
            id="capitanEquipoLocalId"
            v-model="matchForm.capitanEquipoLocalId"
            :options="getTeamPlayers(matchForm.equipoLocalId)"
            optionLabel="nombre"
            optionValue="id"
            placeholder="Seleccione capitán"
            class="w-full"
            :disabled="!matchForm.equipoLocalId"
          />
        </div>
        <div class="p-field">
          <label for="capitanEquipoVisitanteId" class="font-bold mb-2 block">Capitán Visitante</label>
          <Dropdown
            id="capitanEquipoVisitanteId"
            v-model="matchForm.capitanEquipoVisitanteId"
            :options="getTeamPlayers(matchForm.equipoVisitanteId)"
            optionLabel="nombre"
            optionValue="id"
            placeholder="Seleccione capitán"
            class="w-full"
            :disabled="!matchForm.equipoVisitanteId"
          />
        </div>
      </div>
