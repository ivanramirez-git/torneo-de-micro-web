<template>
  <!-- Goal Dialog -->
  <Dialog v-model:visible="goalDialogModel" header="Registrar Gol" modal class="p-fluid">
    <div class="space-y-4">
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Equipo</label>
        <Dropdown v-model="goalForm.teamId" :options="availableTeams" optionLabel="nombre" optionValue="id"
          placeholder="Seleccione un equipo" class="w-full" />
      </div>
      <div class="p-field">
    ...
    <template #footer>
      <Button label="Cancelar" icon="pi pi-times" @click="closeGoalDialog" class="p-button-text" />
      <Button label="Guardar" icon="pi pi-check" @click="saveGoal" autofocus />
    </template>
  </Dialog>

  <!-- Foul Dialog -->
  <Dialog v-model:visible="foulDialogModel" header="Registrar Falta" modal class="p-fluid">
    <div class="space-y-4">
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Equipo</label>
        <Dropdown v-model="foulForm.teamId" :options="availableTeams" optionLabel="nombre" optionValue="id"
          placeholder="Seleccione un equipo" class="w-full" />
      </div>
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Jugador</label>
        <Dropdown v-model="foulForm.playerId" :options="getTeamPlayers(foulForm.teamId)" optionLabel="nombre"
          optionValue="id" placeholder="Seleccione un jugador" class="w-full" />
      </div>
    </div>
    <template #footer>
      <Button label="Cancelar" icon="pi pi-times" @click="closeFoulDialog" class="p-button-text" />
      <Button label="Guardar" icon="pi pi-check" @click="saveFoul" autofocus />
    </template>
  </Dialog>

  <!-- Card Dialog -->
  <Dialog v-model:visible="cardDialogModel" header="Registrar Tarjeta" modal class="p-fluid">
    <div class="space-y-4">
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Equipo</label>
        <Dropdown v-model="cardForm.teamId" :options="availableTeams" optionLabel="nombre" optionValue="id"
          placeholder="Seleccione un equipo" class="w-full" />
      </div>
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Jugador</label>
      ...
      <div class="p-field">
        ...
        <div class="flex gap-4 mt-2">
          <Button label="🟨 Amarilla" severity="warning" :outlined="selectedCardType !== 'yellow'"
            @click="selectedCardType = 'yellow'" />
          <Button label="🟦 Azul" severity="info" :outlined="selectedCardType !== 'blue'"
            @click="selectedCardType = 'blue'" />
          <Button label="🟥 Roja" severity="danger" :outlined="selectedCardType !== 'red'"
            @click="selectedCardType = 'red'" />
        </div>
      </div>
    </div>
    <template #footer>
      <Button label="Cancelar" icon="pi pi-times" @click="closeCardDialog" class="p-button-text" />
      <Button label="Guardar" icon="pi pi-check" @click="saveCard" autofocus />
    </template>
  </Dialog>

  <!-- Time Request Dialog -->
  <Dialog v-model:visible="timeRequestDialogModel" header="Solicitud de Tiempo" modal class="p-fluid">
    <div class="space-y-4">
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Equipo</label>
        <Dropdown v-model="timeRequestForm.teamId" :options="availableTeams" optionLabel="nombre" optionValue="id"
          placeholder="Seleccione un equipo" class="w-full" />
      </div>
      <div class="p-field">
  ...
  <!-- Penalty Dialog -->
  <Dialog v-model:visible="penaltyDialogModel" header="Registrar Penal" modal class="p-fluid">
    <div class="space-y-4">
      <div class="p-field">
        <label class="block text-sm font-medium text-gray-700">Equipo</label>
        <Dropdown v-model="penaltyForm.teamId" :options="availableTeams" optionLabel="nombre" optionValue="id"
          placeholder="Seleccione un equipo" class="w-full" />
      </div>
      <div class="p-field">
    ...
    <template #footer>
      <Button label="Cancelar" icon="pi pi-times" @click="closePenaltyDialog" class="p-button-text" />
